...
router.get('/scrapyard', async (req, res) => {
  try {
    ...
    // Limit to 20 items to reduce memory usage
    ...
    // Get all comment counts in a single query instead of multiple queries
    const itemIds = scrapyardItems.map(item => item.id);
    ...
    if (itemIds.length > 0) {
      ...
      if (commentsError) {
        console.error('Error fetching comments for scrapyard items:', commentsError);
      } else if (commentCounts) {
        // Create a map of topic_id to comment count
        commentCountMap = commentCounts.reduce((acc, comment) => {
          acc[comment.topic_id] = (acc[comment.topic_id] || 0) + 1;
          return acc;
        }, {});
      }
    }

    // Map items with comment counts
    const enhancedItems = scrapyardItems.map(item => ({
      id: item.id,
      title: item.title,
    ...
    // Clear large variables to help with garbage collection
    html = null;
