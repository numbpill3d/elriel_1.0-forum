...
router.post('/save', isAuthenticated, (req, res) => {
  try {
    const { svgData, audioData, seed } = req.body;
    ...
    // Insert glyph into database
    const result = db.prepare(`
      INSERT INTO glyphs (user_id, svg_data, audio_data, seed)
      VALUES (?, ?, ?, ?)
    `).run(req.session.user.id, svgData, audioData, seed);
...
function generateGlyphSVG(seed, complexity) {
  ...
  for (let i = 0; i < numGlitches; i++) {
    ...
    const width = 10 + random() * 40;
    const height = 2 + random() * 5;
    svg += `<rect x="${x}" y="${y}" width="${width}" height="${height}" fill="#ff0066" opacity="${0.1 + random() * 0.3}" />`;
  }
