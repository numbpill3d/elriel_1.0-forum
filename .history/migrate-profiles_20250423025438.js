...
// First, let's check what columns are actually in the Supabase profiles table
async function checkProfilesTableColumns() {
  try {
    // This is a PostgreSQL query to list columns in the profiles table
    const result = await supabaseRequest('GET', 
      '/rest/v1/rpc/inspect_profiles_columns', 
      { table_name: 'profiles' });
    
    console.log('Profiles table columns in Supabase:', result);
    return result;
  } catch (error) {
    console.error('Error getting profiles table columns:', error);
    
    // If the RPC call fails, let's try a different approach
    console.log('Creating RPC function to inspect columns...');
    
    try {
      // Create a PostgreSQL function to inspect columns
      await supabaseRequest('POST', '/rest/v1/rpc/create_inspect_columns_function', {});
      console.log('RPC function created successfully');
      
      // Try again with the new function
      const result = await supabaseRequest('GET', 
        '/rest/v1/rpc/inspect_columns', 
        { table_name: 'profiles' });
      
      console.log('Profiles table columns in Supabase:', result);
      return result;
    } catch (innerError) {
      console.error('Failed to create or call RPC function:', innerError);
      return null;
    }
  }
}
