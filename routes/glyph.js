...
// Try to connect to the database, but handle different database types
let db;
try {
  // Check if we're using Supabase
  if (process.env.SUPABASE_URL && process.env.SUPABASE_KEY) {
    console.log('Using Supabase for glyph data');
    const { createClient } = require('@supabase/supabase-js');
    const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_KEY);

    // Create a wrapper to make Supabase work like SQLite
    db = {
      prepare: (query) => {
        ...
        // Extract table name - this is a simple parser and might need improvement
        let tableName = '';
        if (isSelect) {
          ...
          const tableMatch = query.match(/(update|delete from)\s+(\w+)/i);
          if (tableMatch) tableName = tableMatch[2];
        }
        ...
        return {
          ...
          get: (...params) => {
            ...
            // For simplicity, we're just getting the first item
            return supabase
              .from(tableName)
              .select('*')
              .limit(1)
          ...
          run: (...params) => {
            ...
            if (isInsert) {
              // Extract columns and values - this is a simplified parser
              ...
              return supabase
                ...
                .then(({ data, error }) => {
                  ...
                  return { changes: 1, lastInsertRowid: data ? data[0].id : 0 };
                })
                .catch(err => {
    ...
    // Use SQLite if Supabase is not configured
